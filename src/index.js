...
function getCoords(i){
  const columns=7;
  const scale=75;
  var a= Math.floor(i%columns);
  a=a*scale;
  var b = Math.floor(i/columns);
  b=b*scale
  const c = [a,b];
  return c
...
class Game extends React.Component {
  ...
  render() {
    ...
    const id= history[this.state.id]
    const winner = calculateWinner(current.squares);

    const moves = history.map((step, move,id) => {
      const desc = move ? "Go to move #" + move+": "+ this.state.id : "Go to game start";
      return (
        <li key={move}>
    ...
    let status;
    if (winner) {
      status = "Winner: " + (this.state.blueIsNext ? "red": "blue");
    } else {
